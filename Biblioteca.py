...
# (Uso de cadenas de texto, operadores, for, if y variables)
    ...
    #Sacamos la clave de la entrada de texto del buscador
    ...
    if len(clave) >= 1: # Asegurarse de que haya escrito texto.
        ...
        for line in handle: # Checamos cada linea del archivo
            ...
            # Comprobamos que la palabra clave esté en la linea
            if clave in line:
                ...
                # Añadimos todo lo que hayamos encontrado al espacio de texto
                #en la ventana.
        ...
        # Agregaremos la busqueda hecha al historial de busquedas.
...
# (Uso de variables, archivos y cadenas de texto)
    ...
    # Sacamos el nombre del Autor de la entrada de texto.
    autor = agregarAutorE.get()
    titulo = agregarTituloE.get() # Tambien el Título.
    año = agregarAnoE.get() # Y por último el Año.
    # Abrimos el documento donde estan todos los libros en modo append.
    ...
    # Avisamos que se añadió.
    agregarAnadidoL.grid(row=6, column=2, columnspan=2)
    # Borrar todo lo que esté en las entradas de texto.
...
# (Uso de variables, for y cadenas de texto)
    ...
    for line in handle: # Consigue cada una de las lineas/libros.
        ...
        # Mostramos en el bloque de texto cada una de los libros
        # en la biblioteca.
...
# (Uso de archivos, variables, diccionarios, for y cadenas de texto)
    ...
    for line in handle: # Checamos cada linea/libro
        ...
        # Añadimos el Autor y # de libro al diccionario,
        # si no existe empieza de 0.
        dic[y] = dic.get(y,0) + 1
    # Ordena todo de manera alfabética.
...
#Recibe la clave buscada y los libros encontrados.
# (Uso de variables, listas y listas anidadas)
    ...
    # Conseguimos la zona horaria local
    zona_horaria = timezone('America/Monterrey')
    # Sacamos la fecha y hora de la ZHL.
    fecha_hora = datetime.now(zona_horaria)
    # Se le da un formato
    fecha_hora_formato = fecha_hora.strftime("%B %d, %Y %H:%M:%S")
    # lista con la fecha/hora, la clave buscada y los libros encontrados.
    value = [fecha_hora_formato, clave, reps]
    # Se agregan a la lista hitorial. SE OCUPAN LISTAS ANIDADAS.
...
# (Uos de variables, for, listas anidadas y cadenas de texto)
    ...
    # Conseguimos todas las listas dentro de la lista más grande.
...
buscadorEntry = Entry(root) # Entrada de texto
...
#Esta etiqueta irá al lado de la entrada de texto.
...
# Etiqueta que estará oculta hasta que se añada el libro.
...
#Inicia la variable resumen en formato de entero.
#Guardará el estatus del checkbox.
resumen = IntVar()
# Botón para iniciar la vista de la biblioteca.
verButton = Button(root, text='Ver todo', command=ver)
verCheck = Checkbutton(root, text='Resumir', variable = resumen,
onvalue=1, offvalue=0)
# Creamos el checkbox el cual puede estar seleccionado o no,
# y es evalor se guardará en la variable resumen.
...
# Esto solo mantendrá el programa abierto hasta que salga manualmente.
root.mainloop()
